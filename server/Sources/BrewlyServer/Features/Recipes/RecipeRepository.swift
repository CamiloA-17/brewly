...
struct PostgresRecipeRepository: RecipeRepository {
    ...
    private static let summarySelect = """
        SELECT r.id, r.title, r.method_slug, r.dose_g::float8 AS dose_g, r.ratio::float8 AS ratio,
               ...
               \(PageCursor.sqlTimestamp("r.created_at")) AS cursor_created_at,
    ...
    private static let detailSelect = """
        SELECT r.id, r.method_slug, r.forked_from_id, r.title, r.description,
    ...
    func create(authorID: UUID, _ recipe: UpsertRecipeRequest) async throws -> RecipeDTO {
        try await database.transaction { tx in
            ...
            guard let row = try await sql.raw("""
                INSERT INTO recipes
                    (author_id, bean_id, method_slug, title, description, dose_g, water_g, yield_g, grind_size,
                     ...
                     notes, visibility)
                VALUES
                    (\(bind: authorID), \(bind: recipe.beanId), \(bind: recipe.methodSlug), \(bind: recipe.title),
